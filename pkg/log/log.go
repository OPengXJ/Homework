...
// 为elasticsearch实现zap日记
type EsErrorLog struct {
	logger *zap.Logger
}

func (el EsErrorLog) Printf(format string, v ...interface{}) {
	el.logger.Sugar().Errorf(format, v...)
}
func NewEsError() *EsErrorLog {
	return &EsErrorLog{
		logger: log,
	}
}

type EsInfoLog struct {
	logger *zap.Logger
}

func NewEsInfo() *EsInfoLog {
	return &EsInfoLog{
		logger: log,
	}
}

func (el EsInfoLog) Printf(format string, v ...interface{}) {
